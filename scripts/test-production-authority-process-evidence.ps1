...
if([string]$c.mode-ceq'request_mutation'){Start-Sleep -Milliseconds 100;[byte[]]$mutation=$utf8.GetBytes(" `n");$changed=$false;for($attempt=0;$attempt-lt200-and-not$changed;$attempt++){try{$append=New-Object IO.FileStream([string]$c.request,[IO.FileMode]::Append,[IO.FileAccess]::Write,[IO.FileShare]::None);try{$append.Write($mutation,0,$mutation.Length);$append.Flush($true)}finally{$append.Dispose()};$changed=$true}catch [IO.IOException]{if($append){$append.Dispose()};Start-Sleep -Milliseconds 10}};if(-not$changed){exit 44}}
