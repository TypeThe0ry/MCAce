...
$requestBytes=$null
while($null-eq$requestBytes){if([DateTimeOffset]::UtcNow-ge$deadline){exit 42};if([IO.File]::Exists([string]$c.request)){try{$requestBytes=[IO.File]::ReadAllBytes([string]$c.request)}catch [IO.IOException]{$requestBytes=$null}};if($null-eq$requestBytes){Start-Sleep -Milliseconds 50}}
...
if([string]$c.mode-ceq'request_mutation'){[byte[]]$mutation=$utf8.GetBytes(" `n");$append=New-Object IO.FileStream([string]$c.request,[IO.FileMode]::Append,[IO.FileAccess]::Write,[IO.FileShare]::None);try{$append.Write($mutation,0,$mutation.Length);$append.Flush()}finally{$append.Dispose()}}
