...
Status snapshot: August 22, 2026. Files below `build/` are mutable diagnostics.
...
| GitHub Actions / exact commit | Passed for current canonical main commit `14e44dc52861486b1145078c86cbfac0af6617d0` | Run [`32564457704`](https://github.com/TypeThe0ry/MCAce/actions/runs/32564457704) passed in 5m23s. It verified the exact checkout, ran build/test, built `releaseBundle`, uploaded the eight-file bundle, and produced `release_identity=true` with `source_commit=14e44dc52861486b1145078c86cbfac0af6617d0`; local SHA256 verification passed. See [`release-bundle-2026-08-22-final.json`](evidence/release-bundle-2026-08-22-final.json). | This gate is green for current canonical `main`; the product version remains `0.1.0-SNAPSHOT` until the external v0.0.1 gates close. |
...
| Real server anti-cheat detection/interception | Passed for real 1.21.11 loopback server | [`anti-cheat-real-server-2026-08-22.json`](evidence/anti-cheat-real-server-2026-08-22.json); Leaf 1.21.11 with real GrimAC `2.3.74-155abaf` emitted `Simulation`/`TickTimer` flags, MCAce mapped them to `SERVER_CONFIRMED` `BEHAVIOR_HIGH_RISK`, and the Cloud risk channel returned HTTP `202` twice. | Detection and interception/upload are proven. `MONITOR`/`NONE` is intentional; no automatic kick/ban is asserted. Vulcan and GUI/federation gates remain separate. |
...
| SERVER_CONFIRMED producer | Grim real-process path passed; production freeze pending | [`anti-cheat-real-server-2026-08-22.json`](evidence/anti-cheat-real-server-2026-08-22.json) proves a real Grim provider and real Paper process reached the MCAce `SERVER_CONFIRMED` channel. The default-disabled codec/registry/journal/coordinator foundation remains unit-tested. | Provider/profile/key/topology freeze and the genuine licensed Vulcan producer are still pending; the loopback evidence is not a production Cloud claim. |
