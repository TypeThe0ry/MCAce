...
`5a7e423b5b7bc6c79ea5e1fd3182b82923312169`; it is a documentation/evidence-only
...
| GitHub Actions / exact commit | Passed for feature source `5a7e423b5b7bc6c79ea5e1fd3182b82923312169`; protected branch/tag release path pending | [Push run `32803002956`](https://github.com/TypeThe0ry/MCAce/actions/runs/32803002956) and [PR run `32803006026`](https://github.com/TypeThe0ry/MCAce/actions/runs/32803006026), with sanitized [evidence](evidence/github-feature-ci-2026-08-25-5a7e423.json), passed exact checkout, build/test, local-verification bundle, and exact-eight upload; releaseBundle/readiness are deliberately restricted to protected `main` and `v0.0.1` tag refs. | Current feature witness is green. Merge to protected `main` only after external gates close, then verify the `main` bundle and rerun the same exact-commit release path on the `v0.0.1` tag before publishing. |
| Current fail-closed readiness | Blocked for feature source `5a7e423b5b7bc6c79ea5e1fd3182b82923312169` | [`release-readiness-2026-08-25-5a7e423.json`](evidence/release-readiness-2026-08-25-5a7e423.json) records the latest complete fail-closed evaluation: server matrix and clean worktree passed; the feature Helio candidate is explicitly not protected-main (`protected_main_exact_commit_ci=false`), so the protected branch/tag exact bundle gate remains false alongside GUI six decisions, federation handoff, licensed Vulcan genuine event, and production authority freeze. The gate validates the live `build/release-bundle` plus 3/3 compatibility report only in protected `main` or `v0.0.1` tag push CI context. | No tag/release claim is valid while any named blocker remains open. Rerun `scripts/release-readiness.ps1` after every evidence change. |
| GitHub branch/tag protection | Passed | [`github-protection-2026-08-25.json`](evidence/github-protection-2026-08-25.json) records strict `main` protection with required `build`, operator-only `v0.0.1` tag creation, and active tag deletion/update rules with `current_user_can_bypass=never`. | Repository policy is configured; the protected `main`/`v0.0.1` exact-commit CI still has to run after the external gates close. |
...
| Helio exact-commit release candidate | Passed for feature source `63ae400adc8d09d8349ca599c9d6a4866a189d04` | [`release-bundle-2026-08-25-63ae400.json`](evidence/release-bundle-2026-08-25-63ae400.json) records Helio `releaseBundle`: six deployables plus `release-manifest.properties` and `SHA256SUMS`; remote artifact hashes, `product_version=0.0.1`, root JDK `21.0.10`, modern JDK `25.0.4.1`, Gradle `9.6.1`, and the three-version compatibility contract all passed. | Feature-source candidate is green; it is not protected-main CI and cannot be tagged until GUI, federation, Vulcan, production authority, and current-head main gates close. |
