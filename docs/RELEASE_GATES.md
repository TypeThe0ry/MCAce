...
| Helio exact-commit release candidate | Passed for immutable source `fe5f2d1fba35d0f6222d8bf7f8a90a5b4894e97c` | Helio `releaseBundle` produced six deployables plus `release-manifest.properties` and `SHA256SUMS`; bundle ZIP SHA-256 `3faefd15…`; root JDK `21.0.10`, modern JDK `25.0.4.1`, Gradle `9.6.1`; all eight entries were independently verified locally. | Candidate build is green; it is not a protected-main CI result and cannot be tagged until the GUI, federation, Vulcan, and production gates close. |
...
| Current-head regression suite | Passed 15/15 locally; targeted post-fix server-matrix and preparer gates passed on Helio source-equivalent checkout; exact-checkout GitHub push run [`32615615400`](https://github.com/TypeThe0ry/MCAce/actions/runs/32615615400) and PR run [`32615617012`](https://github.com/TypeThe0ry/MCAce/actions/runs/32615617012) passed for tested source `27bb101…`; subsequent changes are documentation-only | [`static-regression-2026-08-23-current.json`](evidence/static-regression-2026-08-23-current.json) records the script hashes and bounded worker evidence. GUI, Vulcan, federation, and production topology gates remain open. |
...
| Paper/Folia × Velocity/Bungee version process matrix | Passed 12/12 | [`server-version-process-matrix-2026-08-23-current.json`](evidence/server-version-process-matrix-2026-08-23-current.json) with the executed binding/commit triplet; current-source `-Execute` and `-ReportOnly` both passed, all 12 cases, 10 stable + 2 beta, cleanup zero; source manifest `749c21e7…` / 688 files, bound to `eb36229…`. | Signed admission and shadow backend context are proven on all three versions while the peer remains live. Folia 26.2 build 4 remains the two-case BETA lane. Online-mode/public-network behavior is not claimed. |
...
| Current exact-release candidate anti-cheat rerun | Passed against the current release candidate | [`current-candidate-fe5f2d1.json`](evidence/real-server-2026-08-23/current-candidate-fe5f2d1.json) binds source `fe5f2d1…`, Paper artifact `9bb12776…`, Leaf `1.21.11-135`, and GrimAC `2.3.74-155abaf`: Node.js 22.23.2 connected, 40 probes completed, and three `SERVER_CONFIRMED` events were uploaded with HTTP 202. | Current candidate detection/interception is green; `MONITOR`/`NONE` remains intentional. This does not close licensed Vulcan, GUI/federation, or production topology. |
